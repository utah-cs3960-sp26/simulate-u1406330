...
bool testPackedSceneHonorsRequestedBallCount() {
    ...
    return expectNear(scene.balls.front().radius, 6.0, 1e-12,
                      "packed scene should keep the configured radius");
}
...
bool testCsvRoundTrip() {
    ...
    for (std::size_t i = 0; i < scene.balls.size(); ++i) {
        ...
        if (!expectNear(actual.position.x, expected.position.x, 1e-12, "csv x mismatch") ||
            ...
            !expect(actual.spawnFrame == expected.spawnFrame, "csv spawn frame mismatch") ||
...
bool testCsvPreservesWalls() {
        ...
        std::filesystem::temp_directory_path() / "simulate_u1406330_minimal_scene.csv";
    {
        ...
        out << "x,y,previous_x,previous_y,spawn_frame,vx,vy,r,g,b,radius\n";
        out << "20,30,18.75,30.5,5,1.25,-0.5,255,0,0,5\n";
        out << "60,80,62,79.25,0,-2.0,0.75,0,255,0,7\n";
    }
...
constexpr TestCase kTests[] = {
    ...
    {"packed_scene_honors_requested_ball_count", &testPackedSceneHonorsRequestedBallCount},
